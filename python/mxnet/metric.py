...
#所有指标的父类，self_num指的是有多少个指标可以计算，比如说可以将accuracy和loss放在这里一起来计算
#同时，这里的name可以是一个list,num就是评级指标多少个，长度和num相等
class EvalMetric(object):
    ...
    ##应该也是一个list,inst也是一个list,其中de 
    #num_inst指的是总共有多少的数据，sum_metric是指满足这个指标的有多少的数据
    def reset(self):
...
class Accuracy(EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        for label, pred_label in zip(labels, preds):
            ...
            check_label_shapes(label, pred_label)#多余的判断吗？
...
#自己定义的evalmetric
#feval自己定义的评价函数，_allow_extra_outputs是否允许额外的参数
#照着上面的来就可以了
